...
- **Payment Model Overhaul:** Removed Stripe Connect (seller Express accounts). Platform now operates a single Stripe account. All buyer payments collected by platform. Sellers receive bank settlements. Sellers provide bank/UPI details instead of Stripe credentials.
- **UI/UX Design References:** Added Section 0 — Design System & UI/UX References with inspiration sources, tokens, and component guidelines.
...
5. Create Checkout Session:
   stripe.checkout.sessions.create({
     ...
     // NOTE: No transfer_data or application_fee_amount.
     // Platform Stripe account collects 100% of payment.
     // Seller settlement is handled separately via bank transfer (see Section 2.1).
     payment_intent_data: {
       metadata: { userId, agentId, sellerId }
     },
...
2. Event routing:
   checkout.session.completed:
     ...
     → INSERT INTO purchases { 
         amount_paise, 
         platform_fee_paise: Math.round(amount * 0.15),   // Platform keeps 15%
         seller_payout_paise: Math.round(amount * 0.85),  // 85% owed to seller
         settlement_status: 'pending'                      // Settled on weekly cycle
       }
...
1. Query all purchases from last 7 days:
   SELECT seller_id, SUM(seller_payout_paise) as total_payout
   FROM purchases
   WHERE settlement_status = 'pending'
     AND created_at < NOW() - INTERVAL '7 days'   -- 7-day hold (refund window)
     AND refund_status IS NULL
   GROUP BY seller_id

2. For each seller:
   - Fetch seller_bank_details (account_number, IFSC)
   - Deduct any pending refund clawbacks from this cycle
   - Calculate TDS if applicable (10% if earnings > Rs 30,000/year threshold)
   - Initiate bank transfer via:
       Option A: Manual NEFT (admin action on bank portal) — Phase 1
       Option B: Razorpay Payouts API (automated) — Phase 2

3. Record each settlement:
...
4. Email each seller:
   Subject: "Your AI Genius earnings have been transferred"
   Body:
     Period: May 1–7, 2026
     Gross earnings:    Rs 16,110
     TDS deducted:      Rs     0   (below threshold)
     Refund adjustments: Rs    0
     Net transferred:   Rs 16,110
     Account:           HDFC ****4521
     Reference:         NEFT/IMPS Ref# ABC123
     Expected credit:   Within 2 business hours
```
