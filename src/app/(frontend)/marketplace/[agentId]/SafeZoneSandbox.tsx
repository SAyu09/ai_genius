  ...
  return (
    <div className="flex flex-col h-[500px] bg-background shadow-sm">
        ...
        {messages.map((m) => (
          <div key={m.id} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : "flex-row"} ${m.role === "system" ? "justify-center" : ""}`}>
            {m.role === "system" ? (
              ...
              <>
                <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${m.role === "user" ? "bg-indigo-600 text-white" : "bg-indigo-100 text-indigo-600"}`}>
                  {m.role === "user" ? <User className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
                </div>
                <div className="flex flex-col gap-1 max-w-[80%]">
                  <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${m.role === "user" ? "bg-indigo-600 text-white rounded-tr-md" : "bg-muted/50 border rounded-tl-md whitespace-pre-wrap"}`}>
                    {m.content}
                  </div>
      ...
      <form onSubmit={handleSend} className="p-3 bg-muted/20 border-t flex gap-2">
        ...
        />
        <Button type="submit" size="icon" className="rounded-xl h-11 w-11 shrink-0 bg-indigo-600 hover:bg-indigo-700 text-white" disabled={!input.trim() || isTyping || isLimitReached}>
